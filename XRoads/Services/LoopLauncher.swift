...
actor LoopLauncher {
    ...
    private func createAgentMd(config: LoopConfiguration, worktreePath: URL) async -> String {
        ...
        **YOU MUST WORK AT THE ROOT OF THIS WORKTREE.**
        ...
        \(geminiToolGuardrails(config: config))
    ...
    // MARK: - Agent-Specific Guardrails

    /// Gemini CLI has known issues with MCP tool usage. This section injects
    /// explicit guardrails to reduce hallucinated tool names, failed edits, and
    /// forbidden shell patterns.
    private func geminiToolGuardrails(config: LoopConfiguration) -> String {
        guard config.agentType == .gemini else { return "" }
        return """

        ## CRITICAL: Gemini Tool Usage Rules

        You are running as Gemini CLI with MCP tools. Follow these rules **exactly** to avoid errors:

        ### 1. Use EXACT tool names — never guess
        The available tools are: `read_file`, `write_file`, `replace`, `run_shell_command`, `shell_execute`.
        **DO NOT** invent tool names. If unsure, use `run_shell_command`.

        ### 2. ALWAYS read before editing
        Before calling `replace` (edit tool), you **MUST** first call `read_file` on the target file.
        Copy the exact text from `read_file` output into `old_string`. Do NOT type it from memory.
        If `replace` fails with "0 occurrences found", re-read the file and try again with the exact text.

        ### 3. No command substitution in shell
        `$()`, `<()`, and `>()` are **FORBIDDEN** in `run_shell_command`.
        Instead, use pipes, temp files, or multiple sequential commands:
        ```bash
        # WRONG (will be rejected):
        TIMESTAMP=$(date -u +%Y-%m-%dT%H:%M:%SZ)

        # CORRECT:
        date -u +%Y-%m-%dT%H:%M:%SZ > /tmp/ts.txt
        ```
        For jq updates to status.json, write the full command as a single pipeline:
        ```bash
        jq --arg id "US-XXX" --arg ts "$(date -u +%Y-%m-%dT%H:%M:%SZ)" '.stories[$id].status = "complete"' file.json > /tmp/out.json && mv /tmp/out.json file.json
        ```
        If that is also rejected, write a small shell script to a .sh file first, then execute it.

        ### 4. write_file is sandboxed to this worktree
        You can only write files inside: `\(config.worktreePath.path)`
        For files outside (like status.json), use `run_shell_command` with `jq`/`cat`/`mv`.

        ### 5. Stop looping on the same error
        If a tool call fails **twice** with the same error:
        - Re-read the file or re-examine the problem
        - Try a completely different approach
        - If stuck, write what happened to `progress.txt` and move on
