...
struct SkillsBadge: View {
    ...
    var body: some View {
        // No .popover() — SwiftUI's NSPopover crashes (SIGSEGV at 0x0 in
        // PopoverHostingView.updateAnimatedWindowSize) during idle layout
        // passes on macOS.  Tap opens Skills Browser via onTap callback.
        if skillCount > 0 {
            Button {
                onTap?()
            } label: {
