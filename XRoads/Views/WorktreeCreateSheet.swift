...
struct WorktreeCreateSheet: View {
    ...
    /// Optional dismiss handler for overlay presentation
    var onDismiss: (() -> Void)? = nil

    /// Dismiss the sheet using either the provided closure or environment
    ...
    var body: some View {
        ...
        .onAppear {
            // CRITICAL: Force window activation and keyboard focus
            DispatchQueue.main.async {
                NSApp.activate(ignoringOtherApps: true)
                
                // Find and activate the sheet window
                if let window = NSApp.keyWindow ?? NSApp.windows.first(where: { $0.isVisible && $0.isSheet }) {
                    window.makeKey()
                    window.makeFirstResponder(nil) // Clear first responder
                    
                    // Force window to accept keyboard events
...
/// Custom text field style matching Dark Pro theme using ViewModifier approach
...
/// Alternative: View modifier for dark text fields
struct DarkProTextField: ViewModifier {
...
extension View {
    func darkProTextField() -> some View {
        modifier(DarkProTextField())
    }
}
