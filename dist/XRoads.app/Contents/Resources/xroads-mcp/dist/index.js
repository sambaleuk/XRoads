...
function sessionsFilePath(repoPath) {
    return path.join(repoPath, CROSSROADS_DIR, SESSIONS_FILE);
}
function ensureCrossroadsDir(repoPath) {
    const dir = path.join(repoPath, CROSSROADS_DIR);
    if (!fs.existsSync(dir)) {
        fs.mkdirSync(dir, { recursive: true });
    }
}
function loadSessionsFromDisk(repoPath) {
...
function saveSessionsToDisk(repoPath, sessions) {
    ensureCrossroadsDir(repoPath);
    const filePath = sessionsFilePath(repoPath);
    fs.writeFileSync(filePath, JSON.stringify(sessions, null, 2), "utf-8");
}
function upsertSessionOnDisk(session) {
...
function handleUpdateStatus(args) {
    ...
    const existingIndex = state.agents.findIndex((a) => a.agent === agent && a.worktree === worktree);
    if (existingIndex >= 0) {
...
// ============================================================================
function handleRegisterSession(args) {
...
function handleRecordDecision(args) {
...
function handleGenerateHandoff(args) {
    const sessionId = args.sessionId;
    const budgetTokens = args.budgetTokens || 500;
    const session = sessionCache.get(sessionId);
    if (!session) {
        throw new Error(`Session not found: ${sessionId}`);
    }
    ...
    const agentState = state.agents.find((a) => a.agent === session.agentName);
    if (agentState) {
...
function handleGetSession(args) {
    const repoPath = args.repoPath;
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            case "emit_log": {
                const result = handleEmitLog(args);
                return {
                    content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
                };
            }
            case "update_status": {
                const result = handleUpdateStatus(args);
                return {
                    content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
                };
            }
            ...
            case "register_session": {
                const result = handleRegisterSession(args);
                return {
                    content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
                };
            }
            case "record_decision": {
                const result = handleRecordDecision(args);
                return {
                    content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
                };
            }
            case "generate_handoff": {
                const result = handleGenerateHandoff(args);
                return {
                    content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
                };
            }
            case "get_session": {
                const result = handleGetSession(args);
                return {
                    content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
                };
            }
